...
def enhanceddnc(pointlistX, pointlistY):
    ...
    if(arraylen <= 3):
        ...
        xL = pointlistX[:len(pointlistX)/2]
        xR = pointlistX[len(pointlistX)/2:]
        xM = pointlistX[len(pointlistX)/2][0]
        yL = []
        yR = []
        for i in pointlistY:
            if i[0] <= xM:
        ...
        for k in range(len(strip) - 1):
            ...
            while (l < len(strip) and  (strip[l][1] - strip[k][1]) < d):
                if (findDistance(strip[l], strip[k])):
                    minSave = findDistance(strip[l], strip[k])
                l += 1
        return minSave
...
#If the divideandconquer method is called.
