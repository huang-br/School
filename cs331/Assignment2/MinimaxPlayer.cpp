...
int MinimaxPlayer::minimum(int &row, int &col, char player, OthelloBoard *b)
{
	...
	int minimumRow;										
	int minimumCol;
	std::vector<move> successorMoves;						//Creating a list of successorBoards
	if(player == 'X')									//Determining which player it is.
	{
		successorMoves = Successors('X', b);
	}
	else if(player == 'O')
	{
		successorMoves = Successors('O', b);
	}

	if(successorMoves.size() == 0)								//Basecase
	{
		return Utility(b);
	}

	for(i=0; i < successorMoves.size(); i++)						//Minimax algorithm
	{
		if(maximum(row, col, player, successorMoves[i]) > v)
		{
			minimumRow = successorMoves[i].x;
			minimumCol = successorMoves[i].y;
			v = maximum(row, col, player, successorMoves[i]);
		}
	}
...
int MinimaxPlayer::maximum(int &row, int &col, char player, OthelloBoard *b)
{
	...
	int maximumRow;
	int maximumCol;
	std::vector<move> successorMoves;
	if(player == 'X')
	{
		successorMoves = Successors('X', b);
	}
	...
	for(i=0; i < successorMoves.size(); i++)
	{
		if(maximum(row, col, player, successorMoves[i]) > v)
		{
			maximumRow = successorMoves[i].x;
			maximumCol = successorMoves[i].y;
			v = maximum(row, col, player, successorMoves[i]);
		}
	}
