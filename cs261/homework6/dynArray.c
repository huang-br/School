...
void addHeap(DynArr *heap, TYPE node)
{
	...
	TYPE parentValue = heap -> data[parentIndex];

	while(pos > 0 && compare(node, parentValue) == -1)
	{
		swapDynArr(heap, pos, parentIndex);
		pos = parentIndex;
		parentIndex = (pos -1)/2;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if(rightIndex <= max)
	{
		small = _smallerIndexHeap(heap, leftIndex, rightIndex);
...
void sortHeap(DynArr *heap)
{
	...
	for(max = heap -> size -1; max >=0; max--)
	{
		swapDynArr(heap, max, 0);
		_adjustHeap(heap,max -1, 0);
	}
}
