...
void saveList(DynArr *heap, FILE *filePtr)
{
  	/* FIX ME */
	int k;
	for(k=0; k <= heap -> size -1; k++)
	{
		fprintf(filePtr, "%d\t%s\n", heap -> data[k].priority, heap -> data[k].description);
	}
...
void loadList(DynArr *heap, FILE *filePtr)
{
	...
	while(fscanf(filePtr, "%d\t", &k) != EOF)
	{
		...
		TYPE temp = createTask(k, description);
		addHeap(heap, temp);
	}
...
void printList(DynArr *heap)
{
	...
	for(k=0; k <= heap-> size -1; k++)
	{
		printf("%d\t%s", heap -> data[k].priority, heap->data[k].description);
	}
	printf("\n");
}
...
/*  Compare two tasks by priority
    ...
    pre:    none
    post:   none
ret: 	-1 ifpriority of left < priority of right
