...
int main()
{
	...
	//Creating an array of empty, non initialized rooms.
	struct room rooms[NUM_ROOMS];

	//Initializing the rooms
	generateRooms(rooms);
	
	//printRooms(rooms); <- used for debugging.
	
	//Creates a directory with all the approperate files.
...
{
	//Creating a list of the rooms that are taken.
	int namesTaken[TOTAL_ROOMS];

	//Filling the the list with all false values.
...
int checkConnection(int r1, int r2, struct room roomList[])
{
	...
	//Checking if the room is already connected.
	int k=0;
...
void createRoomFiles(struct room roomList[])
{
	...
	//creating a string for the name
	char name[30];

	//Creating the name
	sprintf(name, "huangbr.rooms.%d", id);

	//Creating the directory with all the approperate permissions.
...
void game(struct room roomList[])
{
	...
	struct room *path = malloc(sizeof(struct room *) * 50);
	int counter = 0;
...
int getInput(struct room curRoom)
{
	...
	while(goodInput == 0)
	{
		...
		//If there is a match get the index for the next room.
		if(compare == 0)
		{
			int index = curRoom.connections[j] -> indexNum;
			return index;
		}
		//Getting the time. Sadly no threads...
