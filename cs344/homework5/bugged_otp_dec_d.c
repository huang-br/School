...
void decode(int socket, int processId, int numChars);
...
void decode(int socket, int processId, int numChars)
{
	...
	for(k=0; k < numChars - 1; k++)
	{
		...
		decodedText[k] = letters[sum];
	}
	char decodedMessage[numChars];
	bzero(decodedMessage, numChars);
	sprintf(decodedText, "%s\0", decodedText);
	int n = write(socket, decodedText, numChars - 1);
	unlink(fileName);
	unlink(keyName);
	return;
}
...
void newThread(int socket, int processId)
{
	...
	decode(socket, processId, numChars);
	close(socket);
	return;
}
